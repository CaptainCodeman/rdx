...
export interface Plugin<M extends Model = Model> {
  ...
  onModel?<M extends Model>(store: ModelsStore, name: string, model: M): void
...
export interface ModelsStore<M extends Models = Models> extends Store<ModelsState<M>> {
  dispatch: ModelsDispatch<M> & Dispatch
}

export declare function createStore<C extends Config>(config: C): ModelsStore<ConfigModels<C>>
